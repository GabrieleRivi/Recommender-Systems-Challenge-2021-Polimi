...
class UpgradeBestHybrid(BaseItemSimilarityMatrixRecommender):
    RECOMMENDER_NAME = "UpgradeBestHybrid"

    def __init__(self, URM_train, recommenders: list):
        super(UpgradeBestHybrid, self).__init__(URM_train)

        self.URM_train = check_matrix(URM_train.copy(), 'csr')
        self.Recommender_1 = recommenders[0]
        self.Recommender_2 = recommenders[1]  # IALS RECOMMENDER
        self.Recommender_3 = recommenders[2]  # SLIMELASTICNET RECOMMENDER
        self.W_sparse = None

    def fit(self, alpha=0.5):
        self.alpha = alpha
        self.beta = 0.3023745211799622
        self.gamma = 1.0
